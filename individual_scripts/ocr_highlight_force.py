#!/usr/bin/env python3
#############################################
# Script: ocr_highlight_force.py
# Purpose:
#   This script performs forced OCR on a PDF file or all PDFs in a given folder.
#   It:
#     - Uses ocrmypdf with --force-ocr to recreate all text layers regardless of existing ones
#     - Produces:
#         - OCR-enhanced PDF (ocr_output.pdf)
#         - Sidecar plain text file (ocr_output.txt)
#         - HOCR output file (ocr_output.hocr) for layout info
#         - A visual overlay with highlighted bounding boxes for each word
#         - A full log of the OCR process
#     - All outputs are zipped for easy access
# Output:
...
ocrmypdf --force-ocr \
    ...
    print(f"✅ OCR forced on: {pdf_file.name}")
    visualize_hocr(hocr_output, pdf_file, visual_dir)
...
def main():
    if len(sys.argv) < 2:
        print("Usage: ocr_highlight_force.py <pdf_file_or_folder>")
        return
    ...
    for pdf in pdfs:
        base = Path("ocr_force") / f"{pdf.stem}_{timestamp}"
